...
export default function ResultsPage() {
  ...
  if (error) return (
    <div className="min-h-screen bg-surface flex flex-col items-center justify-center gap-4 text-slate-400">
      <p>Could not load results. The server may still be processing.</p>
      <button onClick={() => setRetries(r => r + 1)} className="btn-primary text-sm">Retry</button>
    </div>
  );
    ...
    <div className="min-h-screen bg-mesh">
      ...
      <nav className="flex items-center justify-between px-6 py-4 border-b border-border/50 bg-surface/80 backdrop-blur sticky top-0 z-10">
        <div className="flex items-center gap-3">
          <Link to="/dashboard" className="btn-ghost text-sm px-3 py-2">&larr; Dashboard</Link>
          <h1 className="font-display font-bold text-white">{session?.quiz?.title} &mdash; Results</h1>
        </div>
              ...
              {leaderboard.map((p, i) => (
                <div key={p.name} className="flex items-center gap-3 px-4 py-3 rounded-xl"
                  ...
                  <span className="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0"
                    style={{
                      background: i === 0 ? '#f59e0b' : i === 1 ? '#94a3b8' : i === 2 ? '#c2772a' : '#2a2a45',
                      color: i < 3 ? '#0f0f1a' : '#94a3b8',
                    }}>
                    {i + 1}
                  </span>
        ...
        {results.map((r, i) => (
          <div key={i} className="card animate-fade-in">
            <div className="flex items-start justify-between mb-4">
              <div>
                <span className="badge bg-brand-500/20 text-brand-400 border border-brand-500/20 text-xs mb-2 inline-block">
                  Q{i + 1} &middot; {r.question.type.replace('_', ' ')}
                </span>
              ...
              <span className="text-slate-400 text-sm ml-4 flex-shrink-0">
                {r.totalResponses} response{r.totalResponses !== 1 ? 's' : ''}
              </span>
            </div>
            {r.question.type === 'word_cloud' ? (
              ...
              <div className="h-48">
                <LiveBarChart
                  data={Object.entries(r.aggregated).map(([label, value]) => ({ label, value }))}
